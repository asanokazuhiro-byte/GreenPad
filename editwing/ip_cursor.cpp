...
{
	VPos clickPos;
	view_.GetVPos( x, y, &clickPos );
	lastDblClkTick_ = ::GetTickCount();
	lastDblClkLine_ = clickPos.tl;
	lastDblClkX_ = x;
	lastDblClkY_ = y;
	hasLastDblClk_ = true;
...
void Cursor::on_lbutton_down( short x, short y, bool shift )
{
	...
	if( !shift )
	{
		...
		if( hasLastDblClk_ )
		{
			...
			const DWORD dt = now - lastDblClkTick_;
			const int dx = (x >= lastDblClkX_ ? x-lastDblClkX_ : lastDblClkX_-x);
			const int dy = (y >= lastDblClkY_ ? y-lastDblClkY_ : lastDblClkY_-y);
			const int limX = ::GetSystemMetrics( SM_CXDOUBLECLK ) / 2;
			const int limY = ::GetSystemMetrics( SM_CYDOUBLECLK ) / 2;
			if( dt <= ::GetDoubleClickTime()
			&& dx <= limX && dy <= limY
			&& clickPos.tl == lastDblClkLine_ )
			{
				tripleClick = true;
				hasLastDblClk_ = false;
			}
		...
		if( tripleClick )
		{
			const DPos lineStart( clickPos.tl, 0 );
			const DPos lineEnd(
				(clickPos.tl+1 < doc_.tln()) ? clickPos.tl+1 : clickPos.tl,
				(clickPos.tl+1 < doc_.tln()) ? 0 : doc_.len(clickPos.tl) );
			VPos vpStart, vpEnd;
			view_.ConvDPosToVPos( lineStart, &vpStart, &clickPos );
			view_.ConvDPosToVPos( lineEnd, &vpEnd, &clickPos );
			lineSelectMode_ = true;
			MoveTo( vpStart, false );
			MoveTo( vpEnd, true );
		}
		else
		{
